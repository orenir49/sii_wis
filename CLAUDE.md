...
The mask fields are **read-only and pulled from the receiver** (`get_masks_fn`). The mask that matters is the one actually applied to the detector; a second editable copy here would only ever be a chance to disagree.

Masks are **node-local**: they live in each node's own `lSPAD_standalone_win64` directory and nowhere else, and the single-pixel option has the master *create* one there. So the master usually has no mask file to open, and the correlator carries the mask **by value** as a `pair_map.MaskSource` (node-side path + contents) rather than resolving a name against a master-side path. `launch_node(mask_sink=...)` captures it at the one moment the master can know what the detector is running: the generated branch already has the bytes it uploaded, the manual-filename branch reads them back over SFTP (a failure there logs but never fails the launch, since the mask is applied either way). `NodePanel._applied_mask` holds it, cleared on lSPAD shutdown.

There is **no fallback to a mask file on the master** — one that disagreed with the node would derive plausible pairs for the wrong pixels — so a mask the master never saw applied (a node launched by hand, or `master.py` restarted while the nodes kept running) fails Derive loudly, pointing at the node's lSPAD directory, rather than deriving 320 pairs from an empty set. Launching that node through the receiver is what fixes it. The tracked copies under `.claude/masks/` are records only; nothing reads them at run time. The status line under the mask fields stays terse (`n1 8 active  |  n2 6 active`): the file names are in the entries above it and the node-side directory is the same every time.
...
Overload policy is **hold**: past the RAM cap the correlator stops draining, freezes, and reports in red. No subsampling, no silent skipping. What an overload *means* depends on whether raw per-pixel timestamps are being kept, pushed into every window via `set_write_to_disk()` (tied specifically to write-mode `timestamps`, not `diffs` — diff-capture never writes `px_*.bin`, so it doesn't change this message) — with writes on a backlog is a delay, with them off it is permanent photon loss.
...
The receiver's write mode is a 3-way radio group — **No save (live only)** / **Save timestamps to disk** / **Save time differences to disk** (`ReceiverGUI.write_mode_var`, `'none' | 'timestamps' | 'diffs'`, `'timestamps'` by default) — replacing what used to be a single checkbox. It reduces to one boolean for `run_session_loop(write_hooked=...)`: only `'timestamps'` writes raw per-pixel data. Not `'timestamps'`, **nothing raw is written at all** — no `px_*.bin`, no sync files, and the output directory is not even created (`'diffs'` behaves identically to `'none'` here). Hooked keys go to their correlator queues only; everything else is discarded. The suppression covers un-hooked pixels deliberately (sparing them would leave the ~1.28 GB/s write path in place whenever the correlator watches a subset, which is the normal case) and the sync keys too: they cost ~370 B/s against ~13 MB/s of timestamps, and with no photons kept there is nothing for an offline offset estimate to be applied to. Writing them was also actively harmful — fresh sync streams in a directory still holding an *earlier* run's `px_*.bin` read as one coherent dataset to any offline tool. Now a data directory either holds a whole run or was never touched, and a pre-existing directory is called out in the log. The mode is read once per data connection — and `run_session_loop` keeps that value for every back-to-back session on it — so the choice is **committed** from the accept onward. All three radios are therefore **disabled** while either node has a data connection open or a session active (`NodePanel.write_flag_is_committed`, `ReceiverGUI._refresh_write_mode_lock`), with an on-screen reason and a one-line log entry on each transition. Locking on `_session_active` as well as the connection matters: START is sent synchronously, seconds before the sender connects back, so without it a toggle could land between the two nodes' accepts and write one node's pixels but not the other's — half a dataset. To change the mode, disconnect a node.

**Save time differences to disk** is the middle ground between the other two: it doesn't write raw timestamps, but the live correlator streams every in-range `tau` it computes for each derived pair — the exact values `_pair_kernel`/`_pair_kernel_diffs` bin, filtered to the histogram's acceptance window, discarding the rest exactly as the plain kernel always did — to one append-only `spad_data/diffs/<suffix>_<stamp>/pair_{p1:03d}_{p2:03d}.bin` per pair (raw int64 tau, ps), opened at `start_with_offset()` and closed at `_disable()`/`_reset()`. These files are **never deleted**, the same ground-truth status `px_*.bin` has under `'timestamps'` mode. On close, `_save_npz()` consolidates them into the same batched `.npz` "Save .npz" already produces, adding `diffs_ps` (one concatenated int64 array, in the same pair order as `px1`/`px2`) and `diffs_offset` (length `n_pairs + 1`) rather than a ragged per-pair object array, so no `allow_pickle` is needed. **Only `correlate_kernel.rebin_diffs(diffs, bin_width, tmax, nbins)` is guaranteed to reproduce a saved run's live histogram bit for bit** — plain `np.histogram(diffs, bin_edges(...))` is NOT interchangeable with it: `bin_edges()` is centered for *display* (its edge midpoints land on `-tmax`, `-tmax+bin_width`, ...), a half-bin off from the kernel's own `floor((tau+tmax)/bin_width)` boundaries, and `np.histogram`'s internal fast path disagrees with that formula on boundary-hugging values. `rebin_diffs` is the one function — mirroring the kernel's own arithmetic exactly — that both offline rebinning at a different bin width and exact reproduction at the same one should go through. Selecting `'diffs'` is gated on the correlator already being Enabled (`MultiCorrelateWindow.is_enabled`, the same property `ReceiverGUI` already reads before calibration) — otherwise there is nothing to capture — but that gate is only re-checked while the mode is unlocked; once committed, `'diffs'` never reverts on its own even if the correlator is later disabled.

Every run's live log is also captured to **`spad_data/log/<stamp>.log`** (`run_log.py`, 16 checks). The stamped file is created empty at START, lines are **buffered in memory during integration** and flushed when it ends — the master is already writing up to ~1.3 GB/s and the sender's TCP window is what must not close. After the flush, lines append straight through, so the soft-stop drain messages and the session summary are kept. In mode `'none'` this file is the run's *only* record; an interrupted run leaves a stamped zero-length file, which is itself informative. In mode `'diffs'` the run log is not the only record — the per-pair `.bin` files under `spad_data/diffs/...` are — but raw per-pixel timestamps are still absent, same as `'none'`.
