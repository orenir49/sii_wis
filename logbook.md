...
- Reconfirmed a real, still-open gap along the way: `correlate_multi.py` never calls `ChannelGraph.status()`, so the "lagging (nothing lost)" status line never appeared during the validation run despite the underlying fix and disk accounting working correctly. Small follow-up, not done today.