...
- Real time acquisition with mask_seven.txt
  ...
  - Ran it: single master pixel (151 vs 151), SB mode on the pre-T-mode `main` branch, ~1 Mcps for 2 hours — no bunching signal, same as under T-mode. Rules out a T-mode-pipeline-specific bug as the sole explanation, but doesn't point to anything else either. **Decision: mark no-master-bunching as undecided.** No obvious suspects; rather than keep chasing it now, work with slave pixels only until the pulsed laser is back from repair and allows more thorough (higher-SNR) testing.
