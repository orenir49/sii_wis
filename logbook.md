...
- Real time acquisition with mask_seven.txt
  ...
  - New plan started: `docs/lagging_node_disk_spill_plan.md` — correlate through a lagging (not dead) node by spilling its partner's held-up backlog to disk instead of excluding and losing it, the mask_ten failure mode from the day before.
