...
The file is a newline-separated list of the pixels to **deactivate**; any
pixel not listed stays active. Entries are **physical sensor locations**
(0–319 — i.e. `PIXMAP` *values*, the same units as the correlator's
"Pixel (loc)" fields), not pix IDs (`PIXMAP` indices).

Returns confirmation when mask programming is complete. (Used by
`ssh_launcher.py`'s `launch_node()`; generated by `gen_mask.py`.)
